...
#4 Visualization
##4.1
1. One significant crique of this visualization is that the circles do not appear to be directly related to sizes based on numerical values -- they appear to have been made by "eyeballing." The proportions seem arbitrary, especially the looking at 22 vs 69 for the age group 45-64. This data would have been much better represented using another plotting method, such as a histogram.

2. Another issue is the colors used in this visualization, they don't benefit the viewer. All age categories are black, women are orange, and men are purple. The only real advantage to assigning these colors is to differentiate men from women and responses based on individual sex versus responses divided by age, and this doesn't seem necessary and does not aid the viewer in understanding the graphic.

3. Finally, a third critique of this visualization is how the data is categorized, it is inconsistent in its treatment of groups/circles. Presented in another format, it would make more sense to divide each age group into responses by men and women rather than separate age groups for each circle responding "yes" and "no" and just grouped by gender. 
...
##4.3
I would change the graph so the boxplots are dodged by color, so I removed the function varwidth = TRUE. It makes the distribution of different colors more easily viewable for the reader.
...
I selected to output the number of each cut, and the results are surprising. I'm surprised by the (right-skewed) distribution of the number of diamonds, especially the high number of ideal cuts in this set (over half of the total). It feels like there may be missing variables in this data set which help tell the story. This leads into the other question, we may need to be wary of these numbers because they may only represent one retailer, or one "cut" of diamond (for instance, round or emerald), or could be from a time when diamonds were in particularly high demand. Or possibly diamonds which were from a dishonest distributor. There are a number of hidden variables in this dataset which could explain this subset.
...
##6.4
My assumption, just looking at the table of data arranged by descending sales (and using common sense), is that it would follow for there to be a relationship between the number of listings and the number of sales. I am not sure how strong the relationship would be due to the number of other variables associated with this question, but I would anticipate a relationship.
...
##6.5
The table below has the proportion of missed sales for each city. There were 548 instances of missed sales overall, and the table is organzied by city. Not all cities had missed sales.
```{r}
...
a. The distributions of the median sales price do appear different when grouped by city, within a range. See the bar graph below:
```{r}
txhousing %>%
  group_by(city, month) %>%
  filter(sales > 500) %>% summarize(med_sales = median(median, na.rm = TRUE)) %>%
  ggplot(aes(x = city, y = med_sales)) +
  geom_bar(stat = "identity")
```

b. Yes, I would like to investigate the highest city, Colin County as well as the lowest city which I found surprising, Corpus Christi.

c. We would want to filter out cities and months with sales less than 500 because, due to the time contraints of the data set, would include the outliers caused by the economic recession and housing market decline in 2006/2007, and the crash in 2008/2009, and subsequent impact on the housing market.
```{r}
txhousing %>%
  group_by(city) %>%
  filter(sales < 500)
